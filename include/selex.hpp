...
namespace gsm {
    ...
    **/
    ...
    /**
     * @brief 2-parameter ascending normal function (plateau after z1)
     * @details Return type will be the same as x, so x:
     *            should be a dvar_vector if sL, z1
     *            should be a dvector     if sL, z1 are doubles
     *
     * @param T x - Independent variable (e.g. age or size) as data vector or dvar vector
     * @param T2 sL - ascending limb width
     * @param T2 z1 - size/age at full selection
     *
     * @return selectivity at values of x. Return type will be same as x.
    **/
   template<class T, class T2>
   inline
   const T asc_normal(const T &x, const T2 &sL, const T2 &z1)
   {
       T2 slp  = T2(5.0);//use pretty steep slope for join
       T selLH = mfexp(-T2(0.5)*square((x-z1)/sL));// ascending limb
       T join  = T2(1.0)/(T2(1.0)+exp(-slp*(x-z1)));//0 for x<s50, 1 for x>s50
       T selex = elem_prod(T2(1.0)-join,selLH) + join;
       return selex;
   }
   ...
   /**
     ...
     * @param T x - Independent variable (e.g. age or size) as data vector or dvar vector
     * @param T2 sL - ascending limb width
     * @param T2 s50 - size/age at full selection
    ...
    **/
    ...
    class AscNormal: public Selex<T>
    {
      ...
      public:
         ...
         * @param z1 - size at dome           (default=1)
         */
          AscNormal(T2 sL = T2(1), T2 z1 = T2(1))
          : m_sL(sL), m_z1(z1) {}

          T2 GetSL()  const { return m_sL; }
          T2 GetZ1() const { return m_z1; }

          void SetSL(T2 &sL) { this->m_sL = sL; }
          void SetZ1(T2 &z1) { this->m_z1 = z1; }
          void SetParams(T2 &sL, T2 &z1){
              this->m_sL = sL;
              this->m_z1 = z1;
          }
           ...
           */
          const T Selectivity(const T &x) const {
              return gsm::asc_normal(x, m_sL, m_z1);
          }
           ...
           */
          const T logSelectivity(const T &x) const {
              return log(gsm::asc_normal(x, m_sL, m_z1));
          }
           ...
           */
          const T logSelexMeanOne(const T &x) const {
              T y = log(gsm::asc_normal(x, m_sL, m_z1));
              y  -= log(mean(mfexp(y)));
              return y;
          }
    };
