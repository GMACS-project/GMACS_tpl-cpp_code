...
#--get convergence information----
  ...
  includeNoStdErrors = TRUE;#--include model cases w/ no std errors?
  if (includeNoStdErrors){
    noConv = "--";
  } else {
...
Model convergence was judged on the basis of a small final maximum gradient and successful estimation of parameter uncertainty information (Table `r LETTERS[tblnoModConvDiags]`). Based on these criteria, `r ifelse(nConv==nCases,"all",paste0(nConv," out"))` of the `r nCases` models converged successfully.  

Models that exhibited at least one parameter estimated at a bound are listed in [@tbl-GMACS-PsAtBs]. 

All parameter values are listed in Tables [-@tbl-GMACS-AllParamEsts-1]-[-@tbl-GMACS-AllParamEsts-`r nPgsEstPars`]. 


```{r GMACS-tblModelConvergence,results='asis',eval=FALSE}
  #--TODO: develop code to list convergence diagnostics
  #--
  ...
  cat(cap);
  fn = file.path(child_path$peek(),
                 "../../../Text/external_artifacts/Description24_02GMACS",
...
#| label: tbl_GMACS-PsAtBs
  tmp = dfrPsAtBs |> 
          dplyr::select(case,estimate=est,lb,ub,type=status,
                        gradient=grd,se,description=par_type) |> 
...
tmp1 = dfrEstPars |> dplyr::mutate(
  ...
  for (i in 1:npgs){
    kbl = wtsQMD::doKbl(tmp1[rw:min(rw+nlns,nrow(dfrEstPars)),],
                        digits=3,
                        position="h",
                        border_right=c(1,3,3+2*(1:nCases)),
                        ltx_font_size=7,
                        replaceNAs="--",
                        adjColSpacing=-6) |> 
...
Fits to retained catch: Figures [-@fig-GMACS-RetCatchFit] and [-@fig-GMACS-RetCatchResids]).

Fits to total catch biomass: Figures [-@fig-GMACS-TotCatchFitCFs], [-@fig-GMACS-TotCatchResidsCFsM], and [-@fig-GMACS-TotCatchResidsCFsF]. 

Fits to Tanner crab bycatch in the groundfish fisheries: Figures [-@fig-GMACS-TotCatchFitGFs] and [-@fig-GMACS-TotCatchResidsGFs]. 

```{r GMACS-getCatchFitData}
##--catch data----
dfrCatFit = wtsGMACS::extractRep1Results(reslst,"Catch_fit_summary") |> 
              #dplyr::filter(!(case %in% noConv)) |>  #--uncomment to include non-converged cases
              dplyr::mutate(
...
Fits to the NMFS survey biomass indices: Figures [-@fig-GMACS-IndexFits] and [-@fig-GMACS-IndexResids]). 

```{r GMACS-getIndexFitData}
##--index catch data----
dfrIndxFit = wtsGMACS::extractRep1Results(reslst,"Index_fit_summary") |> 
               dplyr::filter(!(case %in% noConv)) |> #--uncomment to exclude non-converged
               dplyr::mutate(
...
Predicted retained catch size compositions: Figure [-@fig-GMACS-ZCFits1].

Total catch size compositions from the crab fisheries: Figures [-@fig-GMACS-ZCFits2]-[-@fig-GMACS-ZCFits13]). 

The predicted size compositions for males in the directed fishery: Figures [-@fig-GMACS-ZCFits2] and [-@fig-GMACS-ZCFits4]). 

The predicted size compositions for females in the directed fishery: [@fig-GMACS-ZCFits5]. 

Predicted size compositions for males in the snow crab fishery: Figures [-@fig-GMACS-ZCFits6] and [-@fig-GMACS-ZCFits8]

Predicted size compositions for males in the snow crab fishery: Figures -[@fig-GMACS-ZCFits7]) and([@fig-GMACS-ZCFits9]). 

Predicted size compositions for males in the BBRKC fishery: Figures [-@fig-GMACS-ZCFits10] and [-@fig-GMACS-ZCFits12]. 

Predicted size compositions for females in the BBRKC fishery: Figures [-@fig-GMACS-ZCFits11] and [-@fig-GMACS-ZCFits13].

Predicted size compositions for male Tanner crab taken in the combined gear groundfish fisheries: [@fig-GMACS-ZCFits14]. although none of the fits to the data in these years were very good. 

Predicted size compositions for male Tanner crab taken in the combined gear groundfish fisheries: [@fig-GMACS-ZCFits15].

Predicted size compositions for males taken in the fixed gear groundfish fisheries: [@fig-GMACS-ZCFits16].
Predicted size compositions for males taken in the fixed gear groundfish fisheries: [@fig-GMACS-ZCFits17]). 

Predicted size compositions for males taken in the trawl gear groundfish fisheries [@fig-GMACS-ZCFits18]. 

Predicted size compositions for females taken in the trawl gear groundfish fisheries [@fig-GMACS-ZCFits19]).

Predicted size compositions for the NMFS survey: Figures [-@fig-GMACS-ZCFits20]-[-@fig-GMACS-ZCFits22].  
